...
ARG SPARQL_ENDPOINT=http://mema_fuseki:3030/memav6_demo/query

# Set environment variables for build phase
ENV NEXT_PUBLIC_MEMASTATS_URL=${NEXT_PUBLIC_MEMASTATS_URL}
ENV SPARQL_ENDPOINT=${NEXT_PUBLIC_SPARQL_ENDPOINT}

# Debug: Verify variables are passed
RUN echo "Build ENV - MEMASTATS: $NEXT_PUBLIC_MEMASTATS_URL, SPARQL: $SPARQL_ENDPOINT"
...
ENV SPARQL_ENDPOINT=http://mema_fuseki:3030/memav6_demo/query
