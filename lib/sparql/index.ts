...
const queryCache = new Map<
  string,
  { data: ISparqlResponse; timestamp: number }
>();
  ...
  try {
    ...
    if (cached) {
      ...
      if (age < CACHE_DURATION) {
        logger.debug(
          `Using cached result for ${queryId} (age: ${(age / 1000).toFixed(1)}s)`,
        );
        return cached.data;
      } else {
        logger.debug(
          `Cache expired for ${queryId} (age: ${(age / 1000).toFixed(1)}s)`,
        );
      }
    }
