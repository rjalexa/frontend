    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith(".json")) {
          ...
          const articlesWithRequiredFields = parsed.map(
            (article: RawArticle): IProcessedArticle => {
              return {
                ...article,
                ...
                meta_data:
                  article.meta_data?.map(this.convertMetaDataToEntity) || [],
                highlights:
                  article.mema_highlights?.map(
