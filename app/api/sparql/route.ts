...
export async function GET(request: NextRequest) {
 ...
 try {
   ...
   if (!response.ok) {
     const errorMessage = `SPARQL query failed: ${response.status} - ${response.statusText}`;
   ...
   const errorMessage = error instanceof Error ? error.message : 'Unknown error executing SPARQL query';
   console.error("SPARQL query failed:", errorMessage);
   return NextResponse.json(
     { error: errorMessage },
     { status: 500 },
   );
